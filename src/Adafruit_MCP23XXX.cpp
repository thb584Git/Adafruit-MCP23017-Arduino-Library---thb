...
/**************************************************************************/
/*!
  @brief read register IOCON
  @author added by thb
  @date 2023/04/25 
  @param port 0 for Port A, 1 for Port B (MCP23X17 only).
  @returns content of IOCON as a uint8_t.
*/
/**************************************************************************/
uint8_t Adafruit_MCP23XXX::readIOCON(uint8_t port)
{
  Adafruit_BusIO_Register IOCON(i2c_dev, spi_dev, MCP23XXX_SPIREG,
                                getRegister(MCP23XXX_IOCON, port));
  return IOCON.read() & 0xFF;
}

/**************************************************************************/
/*!
  @brief write register IOCON
  @author added by thb
  @date 2023/04/25 
  @param value configration, port 0 for Port A, 1 for Port B (MCP23X17 only).
  */
/**************************************************************************/
void Adafruit_MCP23XXX::writeIOCON(uint8_t value, uint8_t port)
{
  Adafruit_BusIO_Register IOCON(i2c_dev, spi_dev, MCP23XXX_SPIREG,
                                getRegister(MCP23XXX_IOCON, port));
  IOCON.write(value);
...
/**************************************************************************/
/*!
  @brief bulk read register IODIR
  @author added by thb
  @date 2023/04/25
  @param port 0 for Port A, 1 for Port B (MCP23X17 only).
  @returns content of IODIR as a uint8_t.
*/
/**************************************************************************/
uint8_t Adafruit_MCP23XXX::readIODIR(uint8_t port)
{
  Adafruit_BusIO_Register IODIR(i2c_dev, spi_dev, MCP23XXX_SPIREG,
                                getRegister(MCP23XXX_IODIR, port));
  return IODIR.read() & 0xFF;
}

/**************************************************************************/
/*!
  @brief bulk write register IODIR
  @author added by thb
  @date 2023/04/25
  @param value dir states, port 0 for Port A, 1 for Port B (MCP23X17 only).
  */
/**************************************************************************/
void Adafruit_MCP23XXX::writeIODIR(uint8_t value, uint8_t port)
{
  Adafruit_BusIO_Register IODIR(i2c_dev, spi_dev, MCP23XXX_SPIREG,
                                getRegister(MCP23XXX_IODIR, port));
  IODIR.write(value);
